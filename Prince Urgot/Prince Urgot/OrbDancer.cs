...
namespace Prince_Urgot
{
    internal class OrbDancer : Orbwalking.Orbwalker
    {
        ...
        private float _moveModeLapse = 1;
        ...
        private void WalkAround(GameObject target, float radius)
        {
            if((Game.Time-_lastMoveModeTick) > _moveModeLapse)
            {
                ...
                float limz = (float)Math.Sqrt((double)(radius * radius - x * x));
        ...
        private void DrawingOnOnDraw(EventArgs args)
        {
            if(_moveModeTarget!=null)
            {
                ...
                Obj_AI_Turret tt = ObjectManager.Get<Obj_AI_Turret>().OrderBy(t => t.Distance(_moveModeTarget, true)).First(t => t.Team != _moveModeTarget.Team);
